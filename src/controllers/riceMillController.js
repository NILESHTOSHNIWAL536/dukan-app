...
function payload(body) {
  ...
  return {
    name: cleanString(body.name || body.riceMillName),
    ownerName: cleanString(body.ownerName || body.owner),
...
async function list(req, res) {
  res.json(await RiceMill.find({ owner: req.user.id }).sort({ state: 1, district: 1, name: 1 }));
}

async function create(req, res) {
  const data = payload(req.body);
  if (!data.name || !data.state || !data.district || !data.phones.length) {
    return res.status(400).json({ message: "Rice mill name, state, district and one mobile number are required" });
  }
  res.status(201).json(await RiceMill.create({ ...data, owner: req.user.id }));
}

async function update(req, res) {
  const data = payload(req.body);
  const riceMill = await RiceMill.findOneAndUpdate(
    { _id: req.params.id, owner: req.user.id },
    data,
    { new: true, runValidators: true }
  );
  if (!riceMill) return res.status(404).json({ message: "Rice mill not found" });
  res.json(riceMill);
}

async function remove(req, res) {
  const deleted = await RiceMill.findOneAndDelete({ _id: req.params.id, owner: req.user.id });
  if (!deleted) return res.status(404).json({ message: "Rice mill not found" });
  res.status(204).end();
}
